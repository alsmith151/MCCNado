...
fn compute_molecule_key_from_raw(
    reads: &[noodles::bam::Record],
) -> Result<u64> {
...
fn filter_output_reads(reads: Vec<noodles::sam::alignment::RecordBuf>) -> Vec<noodles::sam::alignment::RecordBuf> {
    
    // Map of ReadName -> (Index in original list, Record)
    // We want to keep RT=1 over RT=0 if names match
    let mut unique_reads: HashMap<String, (usize, noodles::sam::alignment::RecordBuf)> = HashMap::with_capacity(reads.len());
    
    for (i, read) in reads.into_iter().enumerate() {
        let name = read.name().map(|n| n.to_string()).unwrap_or_default();
        
        // Check for RT tag: 'RT' = 'R' 'T'
        let is_reporter = read.data().get(&Tag::new(b'R', b'T'))
            .and_then(|v| v.as_int())
            .map(|v| v == 1)
            .unwrap_or(false);
        ...
        match unique_reads.get_mut(&name) {
            Some((_, existing)) => {
                ...
                let existing_is_reporter = existing.data().get(&Tag::new(b'R', b'T'))
                    .and_then(|v| v.as_int())
                    .map(|v| v == 1)
                    .unwrap_or(false);
...
pub fn deduplicate_bam(bam_path: &str, out_path: &str) -> Result<BamDeduplicationStats> {
    ...
    for (parent_id, reads) in mcc_groups.into_iter() {
        ...
        if seen_molecules.insert(key) {
            ...
            let reads_buf: Vec<_> = reads_raw.iter()
